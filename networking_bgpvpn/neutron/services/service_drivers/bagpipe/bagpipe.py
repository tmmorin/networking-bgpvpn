...
class BaGPipeBGPVPNDriver(service_drivers.BGPVPNDriverDB):
    ...
    def __init__(self, service_plugin):
        ...
        self.admin_ctxt = n_context.get_admin_context()
    ...
    def _retrieve_bgpvpn_network_info_for_port(self, port):
        ...
        bgpvpn_connections = (
            self.bgpvpn_db.find_bgpvpn_connections_for_network(self.admin_ctxt,
                                                               network_id)
        )
        ...
        network_info = get_network_info_for_port(self.admin_ctxt, port_id)
    ...
    def notify_port_updated(self, port):
        ...
        port = _core_plugin.get_port(self.admin_ctxt, port['id'])

        if 'binding:host_id' not in port:
            LOG.warning("cannot determine host_id for port %s (status:%s)\n%s", port['id'], port['status'], port)
            return

        agent_host = port['binding:host_id']

        # FIXME:check port['network:dhcp'] or 'device_owner': u'compute:az-tm-devstack02',
        if port['device_owner'] == 'network:dhcp':
            LOG.info("Owner of port %s is network:dhcp, ignoring")

        if port['status'] == const.PORT_STATUS_ACTIVE:
            bgpvpn_network_info = (
                self._retrieve_bgpvpn_network_info_for_port(port)
            )

            if bgpvpn_network_info:
                port_bgpvpn_info.update(bgpvpn_network_info)

                self.agent_rpc.attach_port_on_bgpvpn_network(self.admin_ctxt,
                                                             port_bgpvpn_info,
                                                             agent_host)
        elif port['status'] == const.PORT_STATUS_DOWN:
            self.agent_rpc.detach_port_from_bgpvpn_network(self.admin_ctxt,
                                                           port_bgpvpn_info,
                                                           agent_host)
        else:
            LOG.debug("no action since new port status is %", port['status'])

    def remove_port_from_bgpvpn_agent(self, port):
        port_bgpvpn_info = {'id': port['id'],
                            'network_id': port['network_id']}
        if 'binding:host_id' not in port:
            LOG.warning("cannot determine host_id for port %s", port['id'])
            return
    ...
    def registry_port_updated(self, resource, event, trigger, **kwargs):
        LOG.debug("registry_port_updated: %s", locals())
        LOG.debug("context: %s", kwargs.get('context'))
        neutron_context = kwargs.get('context')
        port_dict = kwargs.get('port').get('id')
        self.notify_port_updated(port_dict)
        #FIXME incomplete ?

    def registry_port_deleted(self, resource, event, trigger, **kwargs):
        neutron_context = kwargs.get('context')
        port_dict = kwargs.get('port')
        LOG.debug("registry_port_deleted: what should we do ? (%s)", locals())
        self.remove_port_from_bgpvpn_agent(port_dict)
        #FIXME incomplete ?
